...
Runtime-сервис не использует FAISS: embeddings и HNSW-индекс хранятся в
PostgreSQL/pgvector. Упоминания FAISS могут встречаться только в ранних
исследовательских ноутбуках, сохранённых для воспроизводимости экспериментов.
...
дообучения. Финальный BGE-M3 pipeline встроен в асинхронный `model-service`:
сервис получает задания через RabbitMQ, читает новости и историю обработки из
PostgreSQL и сохраняет embeddings, кластеры и оценки новизны обратно в БД.
...
### Масштабирование model-service

Количество GPU- и CPU-воркеров задаётся в `.env`:
...
Например, один GPU-воркер и три CPU-воркера:
...
После изменения количества примените конфигурацию:

```powershell
docker compose up -d
```

Все GPU- и CPU-экземпляры подключаются к одной durable-очереди RabbitMQ как competing
consumers. RabbitMQ передаёт каждое сообщение только одному свободному экземпляру,
а `prefetch_count=1` не позволяет одному consumer заранее забрать несколько тяжёлых
заданий. Поэтому количество обработчиков можно менять без изменений API и формата
заданий.

Текущее GPU-масштабирование имеет практические ограничения:

- в Compose указано `gpus: all`, поэтому каждая реплика видит все доступные GPU;
- каждая реплика отдельно загружает BGE-M3 и занимает собственный объём VRAM;
- на одном GPU реплики конкурируют за память и вычисления, поэтому ускорение не
  гарантировано и при нехватке VRAM возможен OOM;
- Docker Compose автоматически не распределяет реплики по разным GPU;
- параллельные incremental-задания могут одновременно прочитать одно состояние
  истории, поэтому для строгой последовательности обновлений нужен дополнительный
  механизм сериализации или блокировок.

Таким образом, сервис архитектурно поддерживает горизонтальное масштабирование
воркеров через RabbitMQ. Для эффективного production-масштабирования следует
назначать каждой реплике отдельный GPU либо запускать реплики на разных GPU-узлах.

Инструкция по предварительной сборке образов, переходу с GPU на CPU и управлению
числом GPU- и CPU-воркеров:
[Запуск model-service на CPU](docs/model_service_cpu.md).

`DEMO_MODE=true` — разрушительный демонстрационный режим. При каждом запуске
API удаляет прикладную схему, разворачивает её заново и загружает
демонстрационные данные. Для работы с сохраняемой базой используйте
`DEMO_MODE=false`.
...
При запуске API загружает 250 новостей Lenta.ru из
...
Любой перезапуск API при включённом демо-режиме повторяет развёртывание с нуля.
...
Локальный запуск демо без Docker:

```powershell
python -m api --demo
```

Этот запуск также пересоздаёт прикладную базу.
...
Пользователи с ролью publisher или admin могут синхронно загрузить новости как
черновики или сразу отправить их на публикацию:

- `GET /api/v1/news/import-formats` — список доступных форматов;
- `POST /api/v1/news/import` — multipart-запрос с полями `format`, `file` и
  необязательным `publish_immediately`;
- `POST /api/v1/news` принимает JSON-поле `publish_immediately`;
- `POST /api/v1/news/publish` публикует список `article_ids` одним заданием
  pipeline.

Сейчас поддерживается формат `lenta`: CSV с обязательными колонками `title`,
`text` и `date` (либо `published_at`). Дополнительно принимаются `url`, `topic`,
`tags`, `language` и `news_id`. Лимиты одного импорта: 10 000 строк и 50 MiB.
Новый формат добавляется отдельным importer-классом и регистрируется в
`src/news/importers.py`.

При массовой публикации стоимость списывается за каждую реально опубликованную
статью, а модельный сервис получает одно incremental-задание со всеми ID.
Публикация атомарна на уровне БД: если хотя бы одна выбранная статья не может
быть опубликована или не хватает средств, не публикуется ни одна. Для режима
`import + publish` в таком случае откатывается и создание импортированных
черновиков.
...
- горячая обработка тех же 10 000 новостей, что в standalone benchmark:
  - `full`: `226.33` с, или `44.18` новости/с;
  - `incremental`: `697.71` с, или `14.33` новости/с;
- HTTP import 10 000 новостей: `49.47` с; import + pipeline: `747.47` с;
- прогретый semantic search: `0.12–0.17` с серверного времени.

CPU benchmark на AMD Ryzen 9 5900X, PyTorch `2.7.1+cpu`, поднабор 1 000 новостей:

| Конфигурация | Время | Throughput | Ускорение |
|---|---:|---:|---:|
| 1 CPU-контейнер, 1 job × 1 000 | `465.51` с | `2.15` новости/с | `1.00×` |
| 4 CPU-контейнера, 4 jobs × 250 | `407.19` с | `2.46` новости/с | `1.14×` |

Четыре CPU-реплики измеряют агрегатную скорость независимых jobs и не заменяют
глобальный `full` job. Слабое масштабирование связано с конкуренцией 48 PyTorch
threads за 12 физических ядер. Попытка выполнить 10 000 новостей одним CPU job
превысила 30-минутный RabbitMQ acknowledgement timeout, поэтому для воспроизводимого
измерения без изменения конфигурации очереди использован поднабор из 1 000 новостей.
...
- для больших CPU-backfill задач разделить pipeline на parent job и дочерние
  chunked-задачи расчёта embeddings с checkpointing, после чего выполнять одну
  глобальную кластеризацию и novelty classification; до этого не разбивать
  `full` job автоматически, поскольку независимые части изменяют результат модели;
