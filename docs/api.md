...
прочитаны все `.csv`, остальные файлы игнорируются. Также поддерживается
сырой BZip2 CSV (`.bz2`/`.csv.bz2`), включая полный архив Lenta.ru.

Максимум одного upload определяется серверной конфигурацией; текущий demo-контур
допускает файл до 512 MiB и до 1 000 000 строк. Большой импорт выполняется
