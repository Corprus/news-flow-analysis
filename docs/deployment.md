...
- обработка: `PIPELINE_CHUNK_SIZE` для embeddings-пачек, `PIPELINE_AGGREGATE_BATCH_SIZE` для aggregate-пачек и `PIPELINE_HISTORY_WINDOW_DAYS` для ограничения истории по датам текущей пачки;
