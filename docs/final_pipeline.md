# Финальный пайплайн Semantic News Novelty

Этот документ описывает финальную версию пайплайна инференса после экспериментов в
...
- второй проход кластеризации: присоединение к лучшему кандидату `exp_10`;
- модель новизны: `exp_10a_current_model_on_exp10_clustering`.
...
Актуальные метрики новизны на 87 размеченных golden-строках с рантайм-правилом
«первая публикация кластера → `significant`»: precision `0.8471`, recall `0.9863`, F1 `0.9114`.
Улучшение относительно `exp_00b` относится к clustering (`pairwise_f1`
`0.7697 → 0.9017`); метрики новизны при той же модели совпадают.
...
`src/model/` содержит общую экспериментальную и рантайм-логику: кластеризацию, построение
признаков, обучение классификаторов и оценку. `src/final_pipeline/` отвечает за
оркестрацию, близкую к промышленной, и переиспользует этот код, чтобы ноутбук и инференс не
расходились в реализации.

Готовые артефакты финального пайплайна:
...
В `.joblib` лежит небольшая обёртка финальной модели новизны. Модель эмбеддингов
...
Горячий сервисный `full` практически совпал с автономным пайплайном без загрузки
модели: `238.93 − 13.97 = 224.96` с против `226.33` с. Следовательно, основная
причина медленного первого сервисного замера — последовательный инкрементный
алгоритм, а не Docker, RabbitMQ или запись результата в PostgreSQL.

В инкрементном режиме готовая история и её эмбеддинги читаются из PostgreSQL один
раз перед началом задачи. Затем каждая новая публикация обрабатывается
...
контекст для следующей итерации. При батче из 10 000 новых публикаций это приводит
к многократному просмотру растущей истории и повторному копированию DataFrame и
матрицы эмбеддингов через `pd.concat` и `np.vstack`. Поэтому такой сценарий
...
`p_significant=1.0`: это первая публикация кластера, для которой новизна внутри кластера
...
Поле `predictions` содержит только предсказания, которые вызывающий слой должен записать:

- полный режим (`full`) — все запрошенные ID;
- инкрементный режим (`incremental`) — запрошенные ID и более поздние публикации с пересчитанной оценкой новизны при позднем поступлении новости.

Поле `embeddings` соответствует только `embedding_ids`. В инкрементном режиме это
эмбеддинги запрошенных ID; матрица эмбеддингов всей истории наружу не возвращается. `context_ids`
показывает, какие исторические строки использовались только для вычисления.

`versions` содержит `pipeline_version`, модель и ревизию эмбеддингов, версию модели
новизны и версию конфигурации.

`assignments` хранит как итоговое назначение, так и происхождение назначения:
...
2. Для `model_text` считаются эмбеддинги с учётом идентификаторов.
3. Строится строгая бейзлайн-кластеризация `exp_00b`.
4. Строятся candidate pairs для второго прохода.
5. Применяется выбранная конфигурация присоединения `exp_10`:

```text
exp10_src2_sim0.75_days7_m0.03_tj0.15_num1
```

6. Для кластеризованных новостей применяется финальная модель новизны `10a`.

## Инкрементальная обработка

`FinalNewsNoveltyPipeline` остаётся эталонным пакетным пайплайном. Для добавления новых
...
соответствовать её строкам. Эмбеддинги новых строк можно передать через
`new_embeddings`; иначе они рассчитываются энкодером.

Новые публикации обрабатываются хронологически. Пайплайн:

- сохраняет исторические назначения, кроме доказанного бейзлайн-объединения;
- присоединяет публикацию максимум к одному кластеру;
- использует бейзлайн-порог `0.82` и присоединение с учётом подтверждающих признаков из `exp_10`;
...
- рассчитывает новизну на общей истории, но возвращает предсказания только для новых
  публикаций.

Для выбора кластера используется двустороннее временное окно: пайплайн может учитывать
...
Новизна самой поздно поступившей новости по-прежнему рассчитывается только по более раннему
контексту.

Если новая публикация имеет бейзлайн-связь `similarity >= 0.82` сразу с несколькими
кластерами, пайплайн объединяет их. Это воспроизводит связную компоненту бейзлайн-графа,
которую построил бы полный пайплайн. Неоднозначные кандидаты на присоединение ниже бейзлайн-порога
не объединяются.
...
Новизна пересчитывается для всего объединённого исторического кластера.

Без объединения пайплайн пересчитывает новизну всех исторических публикаций выбранного
кластера с более поздним `published_at`. `result.predictions` содержит новые и все
пересчитанные предсказания, а их идентификаторы перечислены в `result.updated_ids`.
...
Для работы финального пайплайна не нужны:

- `data/predictions/*.csv`;
- кеши эмбеддингов из `data/artifacts/embeddings/*.npz`;
- архивы `*.zip`;
- `src/final_pipeline_old/`;
- журналы обучения `notebooks/catboost_info/`.

Эти файлы являются результатами экспериментов или локальными кешами.
