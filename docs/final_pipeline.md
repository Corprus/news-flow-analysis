...
кластера с более поздним `published_at`. `result.predictions` содержит и новые, и
пересчитанные строки, а их ID перечислены в `result.updated_ids`. Назначения публикаций
к кластерам при этом не изменяются.
