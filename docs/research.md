...
`src/final_pipeline/` и `src/model_service/` образуют актуальный runtime-контур.
`src/model/` содержит общие компоненты финального решения и исследовательский
код, который нужен для воспроизводимости ноутбуков: альтернативные
кластеризаторы, MLP/logreg-baselines, projection adapter и старый
`legacy_pipeline`. Эти части не считаются production-функциями MVP, если они не
подключены через `src/final_pipeline/` или `model-service`.
